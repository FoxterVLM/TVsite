        ...
        <div className="movie-characteristics ml-[118px] flex gap-[20px] font-[Manrope]">
          ...
          <div className="right-desc flex flex-col bg-[#1A1A1A] rounded-[12px] p-[50px] w-[519px]">
            ...
            <div className="languages mb-[30px]">
              ...
              <div className="langs-array flex flex-wrap max-w-[419px] gap-[10px]">
                {currentMovie.availableLanguages.map((lang, i) => (
                    <div className='lang font-medium text-white text-[18px] py-[8px] px-[14px] bg-[#141414] border-1 border-[#262626] w-fit rounded-[8px]' key={i}>{lang}</div>
                ))}
              </div>
            ...
            <div className="director mb-[30px]">
              <h6 className='director-title text-[18px] font-medium text-[#999999] mb-[14px]'>Director</h6>
              <div className="director-block p-[14px] bg-[#141414] border-1 border-[#262626] rounded-[8px] flex gap-[10px] items-center">
                <img src={currentMovie.director.img} loading='lazy' className='director-img rounded-[8px]'/>
                <div className="block-text">
                  <h6 className="text-name text-white text-[18px] font-medium">{currentMovie.director.name}</h6>
                  <p className="text-country text-[#999999] text-[16px] font-medium">From {currentMovie.director.country}</p>
                </div>
              </div>
            </div>
            <div className="music">
              <h6 className='music-title text-[18px] font-medium text-[#999999] mb-[14px]'>Music</h6>
              <div className="music-block p-[14px] bg-[#141414] border-1 border-[#262626] rounded-[8px] flex gap-[10px] items-center">
                <img src={currentMovie.music.img} loading='lazy' className='director-img rounded-[8px]'/>
                <div className="block-text">
                  <h6 className="text-name text-white text-[18px] font-medium">{currentMovie.music.name}</h6>
                  <p className="text-country text-[#999999] text-[16px] font-medium">From {currentMovie.music.country}</p>
                </div>
              </div>
            </div>
          </div>
